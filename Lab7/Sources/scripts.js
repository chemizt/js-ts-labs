function useSimpleCalc() {
    ...
    switch (operation) {
        case '+': {
            result.innerHTML = '';
            if (isNaN(calc.a))
                result.innerHTML += 'Некорректно введён первый аргумент. ';
            if (isNaN(calc.b))
                result.innerHTML += 'Некорректно введён второй аргумент.';
            if (!isNaN(calc.a) && !isNaN(calc.b))
                result.innerHTML = 'Результат: ' + calc.add();
            break;
        }
        case '*': {
            result.innerHTML = '';
            ...
            if (isNaN(calc.b))
                result.innerHTML += 'Некорректно введён второй аргумент.';
            if (!isNaN(calc.a) && !isNaN(calc.b))
                result.innerHTML = 'Результат: ' + calc.multiply();
            break;
        }
        default: {
...
function useExtendableCalc() {
    let firstArg = parseInt(document.getElementById('firstInput').value);
    let operation = document.getElementById('secondInput').value;
    let secondArg = parseInt(document.getElementById('thirdInput').value);
    let result = document.getElementById('output');
    class ExtendableCalculator {
        ...
        constructor() {
            ...
            this.methods.set("+", function (a, b) { return a + b; });
            this.methods.set("-", function (a, b) { return a - b; });
        }
    }
    ...
    switch (operation) {
        case '+': {
            ...
            if (!isNaN(firstArg) && !isNaN(secondArg))
                result.innerHTML = 'Результат: ' + calc.calculate(firstArg + ' + ' + secondArg);
            break;
        }
        case '-': {
            result.innerHTML = '';
